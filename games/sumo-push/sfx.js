...
(function () {
  ...
  // Simple step-sequenced loops, synthesized the same way as the SFX
  // primitives above (no audio files) — each track is just a small pattern
  // of tone() calls fired on a fixed interval. Games opt in with
  // SFX.playMusic('trackName') / SFX.stopMusic(). Shared here so any game on
  // the site can add a loop without writing its own scheduler.
  const MUSIC_TRACKS = {
    // Slow, low taiko-drum pulse with a soft high tap accent — a calm "match
    // in progress" bed.
    sumoAmbient: {
      stepMs: 500, steps: 8,
      step(sfx, i) {
        if (i === 0 || i === 4) sfx.tone(78, 0.26, 'sine', 0.16, 38);
        if (i === 2 || i === 6) sfx.tone(108, 0.12, 'sine', 0.09, 55);
        if (i === 6) sfx.tone(640, 0.05, 'square', 0.045, 950);
      }
    },
    // Same voicing, roughly double tempo and a touch higher — reads as
    // urgency for Sudden Death without becoming a different song.
    sumoTense: {
      stepMs: 250, steps: 8,
      step(sfx, i) {
        if (i % 2 === 0) sfx.tone(92, 0.15, 'sine', 0.18, 44);
        if (i % 2 === 1) sfx.tone(132, 0.08, 'sine', 0.11, 70);
        if (i === 0 || i === 4) sfx.tone(720, 0.05, 'square', 0.05, 1050);
      }
    }
  };
  ...
  class ArcadeSFX {
    ...
    _attachAutoUnlock() {
      const unlock = () => {
        this.ensureContext();
        // A game may call playMusic() before the first real gesture unlocks
        // audio (e.g. the instant a match starts). Pick that request back up
        // now that the context actually exists.
    ...
    // ---- Background music ----------------------------------------------
    // Starts (or switches to) a named looping track. Calling this again with
    // the SAME track that's already playing is a harmless no-op, so games
    // can call it every frame/state-change without worrying about restarting
    // the loop or stacking timers.
    playMusic(name) {
      if (!MUSIC_TRACKS[name]) return;
      if (this._musicTrack === name && this._musicTimer) return;
      if (!this.ctx) { this._pendingMusic = name; return; } // wait for the unlock gesture
      this.stopMusic();
      this._musicTrack = name;
      const track = MUSIC_TRACKS[name];
      let step = 0;
      const playStep = () => {
        if (this._musicTrack !== name) return;
        // muted/no-context is handled per-note by tone() itself, so the
        // sequencer just keeps ticking silently and resumes the instant
        // the player unmutes — no separate pause/resume state needed.
        track.step(this, step);
        step = (step + 1) % track.steps;
      };
      playStep();
      this._musicTimer = setInterval(playStep, track.stepMs);
    }
